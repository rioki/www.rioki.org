...
tags:
    - security
---
It is now starting to become common knowledge that, [the longer your password
...
security between the attacker and my bank account is the 6 digit PIN. 
...
Unfortunately the security at that site was not so great and an attacker was 
able to retrieve the user database. Now what the attacker has are a bunch of
...
possible passwords and look if the hash matches or use a big table containing hashes
and associated passwords. It becomes clear why a short password is of a 
disadvantage. In the brute force approach more characters means increasing 
...
one password for everything. If this succeeds the attacker has access to almost
all the services you use. All he needs to do either try the password or 
go though the account retrieval process. Free shopping at amazon or itunes. 
Probably just your email address as a spam source is sufficient. 
...
and asked for only once and that is why I regularly get a block of TANs in the 
mail. 

The only real vulnerability is a successful man in the middle attack, but
in that case password complexity is irrelevant.
